...
//TODO fix od copilota apropo importu randomBytes, bo nwm czemu ale w nodzie sie wypierdala
    ...
    async signIn({ user, account }) {
      ...
      if (account?.provider === "google") {
        ...
        if (!existing) {
          ...
          //TODO: Można coś zrobić z tym hasłem, np. wysłać email z informacją.
          const dummyPassword = await bcrypt.hash(
            randomBytes(32).toString("hex"),
            10
          );
          const profileRef = (await import("@/models/UserProfile"))
            .default;
